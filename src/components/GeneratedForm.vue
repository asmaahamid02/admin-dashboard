        ...
        <v-row>
          <v-col cols="12" v-for="(field, index) in formFields" :key="field.id">
            <v-text-field
              ...
              :rules="
                field.inputRequired ? [(v) => !!v || `${field.inputLabel} is required`] : []
              "              
              :dense="true"
            ></v-text-field>
          </v-col>
...
export default {
  ...
  setup() {
    ...
    const isShowInput = (index = 0) => {
      return (
        formFields.value[index].inputLabel != '' && formFields.value[index].inputLabel != null && 
        formFields.value[index].inputType != '' && formFields.value[index].inputType != null &&
        formFields.value[index].inputPlaceholder != '' && formFields.value[index].inputPlaceholder != null
      )
    }
